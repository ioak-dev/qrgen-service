...
import app.tenant.service as service
...
def create(request):
    ...
    # else:
    #     return JsonResponse(response[1], status=response[0])
    return (200, {'data': 'not implemented'})

@api_view(['GET'])
def get_banner(request, tenant):
    response = service.do_get_banner(tenant)
    return HttpResponse(response[1], status=response[0])

@api_view(['GET'])
def get_tenant(request, tenant):
    response = service.do_get_tenant(tenant)
    return JsonResponse(response[1], status=response[0])

@api_view(['PUT'])
def add_stage(request,tenant):
    response = service.do_update_tenant(tenant,request.body)
    return JsonResponse(response[1], status=response[0])