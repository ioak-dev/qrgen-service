...
def find(space_id, collection_name, search_criteria, sort = None, user_id = None):
    if sort is None:
        data = get_collection(get_dbname(space_id), collection_name).find(declean_object(search_criteria))
    else:
        data = get_collection(get_dbname(space_id), collection_name).find(declean_object(search_criteria)).sort(sort)
    data = list(data)
...
def upsert(space_id, collection_name, data, user_id = None):
    ...
    if data.get('id') is None and data.get('_id') is None:
        ...
        data['createdAt'] = now
        response = get_collection(get_dbname(space_id), collection_name).insert_one(data)
        record = get_collection(get_dbname(space_id), collection_name).find_one({'_id': response.inserted_id})
        ...
        if data.get('id') is None:
            ...
            del data['id']
        updated_record = get_collection(get_dbname(space_id), collection_name).find_one_and_update(
            { '_id' : data.get('_id') },
            { '$set': data },
            new = True
        )
...
def delete(space_id, collection_name, search_criteria, user_id = None):
    ...
    result = get_collection(get_dbname(space_id), collection_name).delete_many(search_criteria)
    return result
